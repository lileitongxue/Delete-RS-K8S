...
function delete_rs(){
	...
	do
	    echo $line
	    #echo $line >> rs.txt
	    sudo kubectl get rs -n $project |grep $line |awk '{print $1, $2, $3, $4}' | while read name num1 num2 num3 
	    do
		if [ "$num1" == "0" -a "$num2" == "0" -a "$num3" == "0" ];then
		    echo `sudo kubectl  get rs $name  -o yaml  -n $project|grep creationTimestamp \
                    |awk 'NR==1'|sed 's/^[ \t]*//g' |cut -d " " -f 2`,$name >> rs.txt
		fi
	    done
	done
	...
	do
	    if [ -f rs.txt ];then
		wordcount=`cat rs.txt|grep $line |sort -nr|awk 'NR>4'|cut -d "," -f 2|wc -l`
                rsname=`cat rs.txt|grep $line |sort -nr|awk 'NR>4'|cut -d "," -f 2`
