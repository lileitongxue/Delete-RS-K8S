...
do
    echo $line
    #echo $line >> rs.txt
    sudo kubectl get rs -n lilei2 |grep $line |awk '{print $1, $2, $3, $4}' | while read name num1 num2 num3 
    do
	if [ "$num1" == "0" -a "$num2" == "0" -a "$num3" == "0" ];then
	    echo `sudo kubectl  get rs $name  -o yaml  -n lilei2|grep creationTimestamp \
